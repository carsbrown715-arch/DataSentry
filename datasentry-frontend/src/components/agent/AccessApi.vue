  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const curlExample = computed(() => {
        ...
        return `# 创建会话
curl -X POST "${base}/api/datasentry/agent/${id}/sessions" \\
  -H "Content-Type: application/json" \\
  -H "X-API-Key: <your_api_key>" \\
  -d '{"title":"demo"}'

# 发送消息
curl -X POST "${base}/api/datasentry/sessions/<sessionId>/messages" \\
  -H "Content-Type: application/json" \\
  -H "X-API-Key: <your_api_key>" \\
...
const baseUrl = '${base}/api/datasentry';
const agentId = ${id};

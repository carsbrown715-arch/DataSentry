    ...
    methods: {
      async loadOptimizationConfigs() {
        try {
          ...
          const response = await fetch(
            `/api/datasentry/prompt-config/list-by-type/${this.promptType}${query}`,
          );
      ...
      async saveConfig() {
        try {
          ...
          const response = await fetch('/api/datasentry/prompt-config/save', {
            method: 'POST',
            headers: {
      ...
      async toggleConfig(config) {
        try {
          const url = config.enabled
            ? `/api/datasentry/prompt-config/${config.id}/disable`
            : `/api/datasentry/prompt-config/${config.id}/enable`;
      ...
      async deleteConfig(configId) {
        ...
        try {
          const response = await fetch(`/api/datasentry/prompt-config/${configId}`, {
            method: 'DELETE',
          });
          const result = await response.json();
      ...
      async batchEnable() {
        ...
        try {
          const response = await fetch('/api/datasentry/prompt-config/batch-enable', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      async batchDisable() {
        ...
        try {
          const response = await fetch('/api/datasentry/prompt-config/batch-disable', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
      ...
      async updatePriority() {
        try {
          const response = await fetch(
            `/api/datasentry/prompt-config/${this.editingPriorityConfig.id}/priority`,
            {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',

...
INSERT INTO datasentry_agent (id, name, description, avatar, status, api_key, api_key_enabled, prompt, category, admin_id, tags, create_time, update_time) VALUES
...
-- 示例数据源可以运行 docker-compose-datasource.yml 建立，或者手动修改为自己的数据源
INSERT INTO datasentry_datasource (id, name, type, host, port, database_name, username, password, connection_url, status, test_status, description, creator_id, create_time, update_time) VALUES 
...
(3, 'product_db', 'h2', 'datasentry_database', 0, 'product_db', 'root', 'root', 'jdbc:h2:mem:datasentry_database;DB_CLOSE_DELAY=-1;DATABASE_TO_LOWER=true;MODE=MySQL;DB_CLOSE_ON_EXIT=FALSE', 'inactive', 'unknown', 'h2测试数据库，包含核心业务数据', 2100246635, NOW(), NOW())
ON DUPLICATE KEY UPDATE name=VALUES(name);

-- 业务知识示例数据（依赖 datasentry_agent）
INSERT INTO datasentry_business_knowledge (id, business_term, description, synonyms, is_recall, agent_id, created_time, updated_time) VALUES
...
-- 语义模型示例数据（依赖 datasentry_agent 和 datasentry_datasource）
INSERT INTO datasentry_semantic_model (id, agent_id, datasource_id, table_name, column_name, business_name, synonyms, business_description, column_comment, data_type, created_time, updated_time, status) VALUES
...
-- 智能体知识示例数据（依赖 datasentry_agent）
INSERT INTO datasentry_agent_knowledge (id, agent_id, title, content, type, is_recall, embedding_status, file_type, created_time, updated_time) VALUES 
...
-- 智能体数据源关联示例数据（依赖 datasentry_agent 和 datasentry_datasource）
INSERT INTO datasentry_agent_datasource (id, agent_id, datasource_id, is_active, create_time, update_time) VALUES 
...
-- 清理策略/规则/绑定示例数据
INSERT INTO datasentry_cleaning_policy (id, name, description, enabled, default_action, config_json, created_time, updated_time) VALUES
(1, '默认清理策略', '内置默认策略', 1, 'DETECT_ONLY', '{"blockThreshold":0.7,"reviewThreshold":0.4,"llmEnabled":true}', NOW(), NOW())
ON DUPLICATE KEY UPDATE name=VALUES(name);

INSERT INTO datasentry_cleaning_rule (id, name, rule_type, category, severity, enabled, config_json, created_time, updated_time) VALUES
(1, '手机号检测', 'REGEX', 'PII_PHONE', 0.90, 1, '{"pattern":"1[3-9]\\\\d{9}","flags":"NONE"}', NOW(), NOW()),
(2, '邮箱检测', 'REGEX', 'PII_EMAIL', 0.90, 1, '{"pattern":"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\\\.[A-Za-z]{2,}","flags":"CASE_INSENSITIVE"}', NOW(), NOW())
ON DUPLICATE KEY UPDATE name=VALUES(name);

INSERT INTO datasentry_cleaning_policy_rule (policy_id, rule_id, priority) VALUES
(1, 1, 1),
(1, 2, 2)
ON DUPLICATE KEY UPDATE priority=VALUES(priority);

INSERT INTO datasentry_cleaning_binding (id, agent_id, binding_type, scene, policy_id, enabled, created_time, updated_time) VALUES

...
public interface LogicalRelationMapper {
	 ...
	 * 根据ID查询逻辑外键
	 */
	@Select("SELECT * FROM datasentry_logical_relation WHERE id = #{id} AND is_deleted = 0")
	LogicalRelation selectById(@Param("id") Long id);

	/**
	 * 根据数据源ID查询逻辑外键列表（未删除的）
	 */
	@Select("SELECT * FROM datasentry_logical_relation WHERE datasource_id = #{datasourceId} AND is_deleted = 0 ORDER BY created_time DESC")
	List<LogicalRelation> selectByDatasourceId(@Param("datasourceId") Long datasourceId);

	/**
	 * 插入逻辑外键
	 */
	@Insert("""
			INSERT INTO datasentry_logical_relation
	...
	@Update("""
			<script>
			UPDATE datasentry_logical_relation
			<set>
	 ...
	 * 逻辑删除外键
	 */
	@Update("UPDATE datasentry_logical_relation SET is_deleted = 1, updated_time = NOW() WHERE id = #{id}")
	int deleteById(@Param("id") Long id);

	/**
	 * 逻辑删除数据源下的所有逻辑外键
	 */
	@Update("UPDATE datasentry_logical_relation SET is_deleted = 1, updated_time = NOW() WHERE datasource_id = #{datasourceId}")
	 ...
	 */
	@Select("""
			SELECT COUNT(*) FROM datasentry_logical_relation
			WHERE datasource_id = #{datasourceId}
			  ...
			  AND is_deleted = 0
			""")
	int checkExists(@Param("datasourceId") Long datasourceId, @Param("sourceTableName") String sourceTableName,

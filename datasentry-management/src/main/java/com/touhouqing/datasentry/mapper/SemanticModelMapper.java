...
public interface SemanticModelMapper {
	...
	@Select("SELECT * FROM datasentry_semantic_model ORDER BY created_time DESC")
	List<SemanticModel> selectAll();

	/**
	 * Query semantic model list by datasentry_agent ID
	 */
	@Select("""
			SELECT * FROM datasentry_semantic_model
	 ...
	 */
	@Update("""
			UPDATE datasentry_semantic_model
			SET status = 1
			WHERE id = #{id}
			""")
	...
	/**
	 * Query semantic models by datasentry_agent ID and enabled status
	 */
	@Select("""
			SELECT * FROM datasentry_semantic_model
			WHERE agent_id = #{agentId}
	...
	@Insert("""
			INSERT INTO datasentry_semantic_model
	...
	@Delete("""
			DELETE FROM datasentry_semantic_model
			WHERE id = #{id}
			""")
	int deleteById(@Param("id") Long id);

	/**
	 * Query semantic models by datasentry_datasource ID, status and table names
	 */
	@Select("""
			<script>
			SELECT * FROM datasentry_semantic_model
			...
			""")
	List<SemanticModel> selectByDatasourceIdAndTableNames(@Param("datasourceId") Long datasourceId,

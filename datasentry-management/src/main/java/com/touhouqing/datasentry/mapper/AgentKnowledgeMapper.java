...
import com.touhouqing.datasentry.entity.AgentKnowledge;
...
public interface AgentKnowledgeMapper {
	...
	@Select("""
			SELECT * FROM datasentry_agent_knowledge WHERE id = #{id} AND is_deleted = 0
			""")
	AgentKnowledge selectById(@Param("id") Long id);

	@Select("""
			    SELECT * FROM datasentry_agent_knowledge WHERE id = #{id}
			""")
	AgentKnowledge selectByIdIncludeDeleted(@Param("id") Long id);

	@Insert("""

			INSERT INTO datasentry_agent_knowledge (agent_id, title, content, type, question, is_recall, embedding_status, source_filename, file_path, file_size, file_type, splitter_type, is_deleted, is_resource_cleaned, created_time, updated_time)
	...
	@Update("""
			<script>
			UPDATE datasentry_agent_knowledge
			<set>
	...
	@Select("""
			<script>
			SELECT * FROM datasentry_agent_knowledge
	...
	@Select("""
			<script>
			SELECT COUNT(*) FROM datasentry_agent_knowledge
			WHERE agent_id = #{queryDTO.agentId}
			<if test="queryDTO.title != null and queryDTO.title != ''">
				AND title LIKE CONCAT('%', #{queryDTO.title}, '%')
			</if>
	...
	@Select("""
			SELECT id FROM datasentry_agent_knowledge WHERE agent_id = #{agentId} AND is_recall = 1 AND is_deleted = 0
			""")

...
public interface DatasourceMapper {
	...
	@Select("SELECT * FROM datasentry_datasource WHERE id = #{id}")
	Datasource selectById(@Param("id") Long id);

	@Select("SELECT * FROM datasentry_datasource ORDER BY create_time DESC")
	List<Datasource> selectAll();

	@Insert("""
			INSERT INTO datasentry_datasource
	...
	int insert(Datasource datasentry_datasource);
	...
	@Update("""
			...
			""")
	int updateById(Datasource datasentry_datasource);

	@Update("UPDATE datasentry_datasource SET test_status = #{testStatus} WHERE id = #{id}")
	int updateTestStatusById(@Param("id") Long id, @Param("testStatus") String testStatus);

	/**
	 * Query data source list by status
	 */
	@Select("SELECT * FROM datasentry_datasource WHERE status = #{status} ORDER BY create_time DESC")
	List<Datasource> selectByStatus(@Param("status") String status);

	/**
	 * Query data source list by type
	 */
	@Select("SELECT * FROM datasentry_datasource WHERE type = #{type} ORDER BY create_time DESC")
	List<Datasource> selectByType(@Param("type") String type);

	/**
	 * Get data source statistics - by status
	 */
	@Select("SELECT status, COUNT(*) as count FROM datasentry_datasource GROUP BY status")
	List<Map<String, Object>> selectStatusStats();

	/**
	 * Get data source statistics - by type
	 */
	@Select("SELECT type, COUNT(*) as count FROM datasentry_datasource GROUP BY type")
	List<Map<String, Object>> selectTypeStats();

	/**
	 * Get data source statistics - by test status
	 */
	@Select("SELECT test_status, COUNT(*) as count FROM datasentry_datasource GROUP BY test_status")
	List<Map<String, Object>> selectTestStatusStats();

	@Select("SELECT COUNT(*) FROM datasentry_datasource")
	Long selectCount();

	@Delete("DELETE FROM datasentry_datasource WHERE id = #{id}")

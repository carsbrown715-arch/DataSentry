...
public interface BusinessKnowledgeMapper {
	...
	/**
	 * Query business knowledge list by datasentry_agent ID
	 */
	@Select("""
			SELECT * FROM datasentry_business_knowledge
	 ...
	 */
	@Select("SELECT * FROM datasentry_business_knowledge WHERE is_deleted = 0 ORDER BY created_time DESC")
	List<BusinessKnowledge> selectAll();

	/**
	 * Search in a specific datasentry_agent scope by keyword
	 */
	@Select("""
			SELECT * FROM datasentry_business_knowledge
			WHERE agent_id = #{agentId} AND is_deleted = 0
	...
	@Insert("""
			INSERT INTO datasentry_business_knowledge (business_term, description, synonyms, is_recall, agent_id, created_time, updated_time, embedding_status, is_deleted)
	...
	@Update("""
			<script>
			UPDATE datasentry_business_knowledge
			<set>
	...
	@Delete("""
			DELETE FROM datasentry_business_knowledge
			WHERE id = #{id}
			""")
	...
	@Select("""
			SELECT id FROM datasentry_business_knowledge

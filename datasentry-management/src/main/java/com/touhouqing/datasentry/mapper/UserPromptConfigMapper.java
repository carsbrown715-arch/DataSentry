...
public interface UserPromptConfigMapper {
	...
	@Select("""
			<script>
			SELECT * FROM datasentry_user_prompt_config
	...
	@Update("""
			<script>
			UPDATE datasentry_user_prompt_config
			SET enabled = 0
			WHERE prompt_type = #{promptType}
			<if test='agentId != null'> AND agent_id = #{agentId}</if>
			</script>
	 ...
	 */
	@Update("UPDATE datasentry_user_prompt_config SET enabled = 1 WHERE id = #{id}")
	int enableById(@Param("id") String id);

	/**
	 * Disable a specified configuration
	 */
	@Update("UPDATE datasentry_user_prompt_config SET enabled = 0 WHERE id = #{id}")
	int disableById(@Param("id") String id);

	@Select("SELECT * FROM datasentry_user_prompt_config WHERE id = #{id}")
	...
	@Insert("""
			INSERT INTO datasentry_user_prompt_config
	...
	@Select("SELECT * FROM datasentry_user_prompt_config ORDER BY priority DESC, display_order, update_time DESC")
	List<UserPromptConfig> selectAll();

	@Delete("DELETE FROM datasentry_user_prompt_config WHERE id = #{id}")
